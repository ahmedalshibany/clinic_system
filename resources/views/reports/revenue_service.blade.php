...
@section('page-i18n', 'revenue_service_report')
...
<style>
    ...
    /* Luxury Financial Color Hierarchy */
    .service-token-total { color: #2e7d32 !important; font-weight: 700; }
    .service-token-count { color: #2e5b5c !important; font-weight: 700; }
    .service-token-top { color: #b07d2a !important; font-weight: 700; }
...
<form action="{{ route('reports.revenue.service') }}" method="GET" id="serviceFilterForm" class="w-100">
    ...
    <div class="card border-0 p-4 mb-4 shadow-sm" style="background: var(--white); border-radius: 16px;">
        <div class="service-filter-wrapper">
            <div class="d-flex flex-wrap gap-2">
                ...
                <button type="button" onclick="applyQuickFilter('this_month') || (!request('quick_filter') && 'this_month')" class="btn btn-sm {{ (request('quick_filter') == 'this_month' || !request('quick_filter')) ? 'btn-primary' : 'btn-light' }} px-3 py-2" style="border-radius: 8px; {{ (request('quick_filter') == 'this_month' || !request('quick_filter')) ? 'background-color: var(--secondary);' : '' }}">{{ __('messages.thisMonth') }}</button>
            ...
            <div class="d-flex gap-3 justify-content-md-end mb-3 mb-md-0">
                <div style="width: 160px;">
                    ...
                    <input type="date" name="date_from" value="{{ $dateFrom }}" onchange="document.getElementById('quickFilterInput').value=''; document.getElementById('serviceFilterForm').submit();" class="form-control form-control-luxury">
                </div>
                <div style="width: 160px;">
                    <label class="form-label text-xs fw-bold text-muted mb-1">{{ __('messages.toDate') }}</label>
                    <input type="date" name="date_to" value="{{ $dateTo }}" onchange="document.getElementById('quickFilterInput').value=''; document.getElementById('serviceFilterForm').submit();" class="form-control form-control-luxury">
                </div>
            </div>
        </div>
    </div>
...
<div class="service-metrics-grid">
    <div class="service-premium-card">
        <span class="text-xs fw-bold text-muted mb-2 d-block">{{ __('messages.total_sales') }}</span>
        <h3 class="service-token-total mb-1"><span class="text-nowrap">{{ $currencySymbol }}{{ number_format($total_sales_sum, 2) }}</span></h3>
        <small class="text-muted text-xs d-block">{{ __('messages.billed_amount') }}</small>
    </div>

    <div class="service-premium-card">
        <span class="text-xs fw-bold text-muted mb-2 d-block">{{ __('messages.items_sold') }}</span>
        <h3 class="service-token-count mb-1" style="font-size: 1.6rem;">{{ $total_items_count }}</h3>
        <small class="text-muted text-xs d-block">{{ __('messages.units_executed') }}</small>
    </div>

    <div class="service-premium-card">
        <span class="text-xs fw-bold text-muted mb-2 d-block">{{ __('messages.top_service') }}</span>
        <h3 class="service-token-top mb-1 text-truncate" style="font-size: 1.3rem; max-width: 100%;" title="{{ $top_service }}">{{ $top_service }}</h3>
        <small class="text-muted text-xs d-block">{{ __('messages.highest_demand') }}</small>
    </div>

    <div class="service-premium-card">
        <span class="text-xs fw-bold text-muted mb-2 d-block">{{ __('messages.active_services') }}</span>
        <h3 class="text-dark fw-bold mb-1" style="font-weight: 700; font-size: 1.6rem;">{{ $service_count }}</h3>
        <small class="text-muted text-xs d-block">{{ __('messages.catalog_tracked') }}</small>
    </div>
</div>
            ...
            <table class="table table-hover align-middle mb-0">
                <thead>
                    <tr class="text-secondary text-xs fw-bold border-0">
                        <th class="py-3">{{ __('messages.serviceName') }}</th>
                        <th class="py-3 text-center">{{ __('messages.qtySold') }}</th>
                        <th class="py-3 text-end">{{ __('messages.totalSales') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($data as $row)
